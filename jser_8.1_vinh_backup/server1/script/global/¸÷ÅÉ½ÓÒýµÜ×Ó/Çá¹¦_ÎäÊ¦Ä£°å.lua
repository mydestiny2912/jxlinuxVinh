...
--	Task0025 = GetTaskTemp(25)			-- ´òÄ¾×®/É³´ü/Ä¾ÈË¼ÆÊı(ÁÙÊ±±äÁ¿)
	...
	tbDailog:AddOptEntry("Ta muèn lµm nhiÖm vô b¸n thŞt t­¬i", education_collectmeat)
...
function education_collectmeat()
	...
	Describe(DescLink_WuShi.."<#>: Ng­¬i hiÖn ®ang cã "..Uworld1065.." miÕng thŞt t­¬i. Ta muèn ng­¬i ra ngoµi rõng t×m thªm mét İt n÷a. Mang vÒ ®©y ta sÏ tÆng ng­¬i mét sè ®iÓm kinh nghiÖm, cã ®ång ı kh«ng? Cã thÓ Ên phİm <color=red>F12<color> ®Ó xem h­íng dÉn chi tiÕt vÒ nhiÖm vô!",
...
function education_givemeat()
	...
	if ( Uworld1066 > 0 ) then
		Describe(DescLink_WuShi.."<#>:Ng­¬i ®· nhËn nhiÖm vô t×m thŞt t­¬i råi! Kh«ng cÇn nhËn n÷a! H·y t×m thªm "..Uworld1066.." miÕng thŞt t­¬i ®i! Cã thÓ Ên phİm <color=red>F12<color> ®Ó xem h­íng dÉn chi tiÕt vÒ nhiÖm vô!",1,"KÕt thóc ®èi tho¹i/no")
	else
...
function education_havemeat()
	...
	if ( Uworld1066 ~= 0 ) then
		if ( Uworld1065 >= Uworld1066 ) then
			...
			Describe(DescLink_WuShi.."<#>:HiÖn giê ta cÇn"..Uworld1066.." miÕng thŞt t­¬i. Ng­¬i hiÖn ®ang cã "..Uworld1065.." miÕng, cè g¾ng t×m thªm ®i!",1,"KÕt thóc ®èi tho¹i/no")
		end
	else 
...
function education_changewuxing()
	...
	if ( Uworld445 ~= -1 ) or ( Uworld446 ~= -1 ) or ( Uworld447 ~= -1 ) or ( Uworld448 ~= -1 ) or ( Uworld449 ~= -1 ) then
		...
		Talk(1,"","Vâ S­: Ha ha! Ta chØ thö ng­¬i th«i! B©y giê ta d¹y ng­¬i c«ng phu thËt sù! Sau nµy nªn lui tíi th­êng xuyªn, ta sª truyÒn cho ng­¬i nh÷ng thø cã İch!") 
		nt_setTask(1067,10)
	else 
