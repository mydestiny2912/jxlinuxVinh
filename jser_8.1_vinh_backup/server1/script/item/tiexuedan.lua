...
function repair_platina(nCount)
	...
	if (GetCurDurability(nItemIndex) == GetMaxDurability(nItemIndex)) then
		CreateTaskSay({"Trang bŞ nµy cã ®ñ ®é bÒn lín nhÊt råi, kh«ng cÇn ph¶i söa thªm", "Thµnh thËt xin lçi, ta sÏ s¾p xÕp l¹i/main", "ThËt ng¹i qu¸, ta sÏ quay l¹i sau./onCancel"});
		return
	end
